...
const stateMachine = function () {
    ...
    const formatDiagnostic = function (token) {
        const reset = "\u001b[0m";
        const fgYellow = "\u001b[33m";
        return "\n  " + fgYellow + token.payload + reset;
    };

    const formatTestline = function (token) {
        const reset = "\u001b[0m";
        const fgGreen = "\u001b[32m";
        const fgBoldGreen = "\u001b[1;32m";
        const fgBoldRed = "\u001b[1;31m";
        if (token.todoDirective !== undefined) {
            if (token.oknotok === "ok") {
                return [
                    "    ",
        ...
        if (token.skipReason !== undefined) {
            return "      " + token.description + " # SKIP " + token.skipReason;
        }
        if (token.oknotok === "ok") {
            return "    " + fgGreen + "✔ " + reset + token.description;
        } else {
            return "    " + fgBoldRed + "✘ " + reset + token.description;
        }
    };

    const formatYaml = function (token) {
        const reset = "\u001b[0m";
        const fgBoldYellow = "\u001b[1;33m";
        return fgBoldYellow + indentLines(6)(token.payload) + reset;
    };
